...
def main_test_basique():
    ...
    """
    print("="*100)
    print("🚀 TEST BASIQUE - OPTIMISATION TRANSPORT UNE SEMAINE")
    print("="*100)
    ...
    try:
        ...
        if resultats:
            ...
            cout_total = resultats['vehicule_optimal']['cout_total']
            vehicule_selectionne = resultats['vehicule_utilise']['nom']
            nb_vehicules = resultats['nb_vehicules_necessaires']
            duree_reelle = resultats['duree_reelle']
            ...
            print(f"⏱️ Durée réelle: {duree_reelle} jour(s) sur {duree_max_jours} autorisés")
            print(f"✅ Contrainte temporelle: {'RESPECTÉE' if resultats['respect_contrainte'] else 'VIOLÉE'}")
            
            # Détails de planification
            planification = resultats['planification_detaillee']
            print(f"\n📋 DÉTAILS DE PLANIFICATION:")
            print(f"📅 Période: {planification['date_debut']} → {planification['date_fin']}")
            print(f"🚚 Voyages totaux: {planification['total_voyages']}")
            print(f"✅ Transport complet: {'OUI' if planification['completion_transport'] else 'NON'}")
            
            # Résumé par véhicule
            print(f"\n📊 RÉSUMÉ PAR VÉHICULE:")
            for planif_vehicule in planification['planifications_vehicules']:
                vehicule_id = planif_vehicule['vehicule_id']
                nb_voyages = planif_vehicule['nb_voyages']
                charge = planif_vehicule['charge_totale']
                
                print(f"  Véhicule {vehicule_id}:")
                print(f"    Voyages: {nb_voyages}")
                print(f"    Charge transportée: {charge['poids']:.2f}t, {charge['volume']:.2f}m³")
                print(f"    Produits transportés:")
                for nom_produit, quantite in planif_vehicule['produits_transportes'].items():
                    print(f"      - {nom_produit}: {quantite} unités")
            
            # Comparaison avec autres véhicules
            print(f"\n📈 COMPARAISON AVEC AUTRES VÉHICULES:")
            for analyse in resultats['analyses_vehicules']:
                vehicule_nom = analyse['vehicule']['nom']
                if analyse['respect_contrainte']:
                    cout_vehicule = analyse['cout_total']
                    difference = cout_vehicule - cout_total
                    if vehicule_nom == vehicule_selectionne:
                        print(f"  ✅ {vehicule_nom}: {cout_vehicule:.2f}€ (SÉLECTIONNÉ)")
                    else:
                        print(f"  💰 {vehicule_nom}: {cout_vehicule:.2f}€ (+{difference:.2f}€)")
                else:
                    print(f"  ❌ {vehicule_nom}: Ne respecte pas la contrainte de {duree_max_jours} jours")
