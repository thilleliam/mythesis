...
class OptimisateurNavettes:
    ...
    def _vehicule_peut_reussir_seul_avec_calcul(self, vehicule, poids_demande, volume_demande, distance_km, duree_max_jours):
        """Version sécurisée du calcul de faisabilité d'un véhicule"""
        try:
            # Validation des entrées
            poids_demande = max(0, float(poids_demande))
            volume_demande = max(0, float(volume_demande))
            distance_km = max(1, float(distance_km))
            duree_max_jours = max(1, float(duree_max_jours))
            
            # Capacités du véhicule
            capacite_poids = vehicule.get('capacite_poids_max', 20.0)
            capacite_volume = vehicule.get('capacite_volume_max', 80.0)
            vitesse = vehicule.get('vitesse_kmh', 80)
            
            # Protection division par zéro
            if capacite_poids <= 0:
                capacite_poids = 20.0
            if capacite_volume <= 0:
                capacite_volume = 80.0
            if vitesse <= 0:
                vitesse = 80
            
            # Calcul voyages nécessaires
            voyages_poids = math.ceil(poids_demande / capacite_poids) if poids_demande > 0 else 0
            voyages_volume = math.ceil(volume_demande / capacite_volume) if volume_demande > 0 else 0
            voyages_necessaires = max(voyages_poids, voyages_volume, 1)  # Minimum 1 voyage
            
            # Estimation temps
            temps_aller_retour = (2 * distance_km) / vitesse
            temps_chargement_dechargement = 3  # 3h par voyage
            ...
            # Calcul coût
            cout_estime = 9999  # Coût par défaut si échec
            if peut_reussir:
                cout_fixe = vehicule.get('cout_fixe_jour', 500) * math.ceil(jours_necessaires)
                cout_variable = distance_km * voyages_necessaires * 2 * vehicule.get('cout_variable_km', 1.0)
            ...
            print(f"❌ Erreur calcul faisabilité véhicule: {e}")
            return {
                'reussite': False,
                'voyages_necessaires': 999,
                'jours_necessaires': 999,
                'cout_estime': 9999
            }
    ...
    def _calculer_efficacite_produit(self, produit):
        """Calcule l'efficacité d'un produit avec protection contre division par zéro"""
        try:
            poids_unitaire = produit.get('poids_unitaire', 0)
            volume_unitaire = produit.get('volume_unitaire', 0.001)  # Valeur minimale pour éviter division par 0
            
            # Validation des valeurs
            if poids_unitaire <= 0:
                poids_unitaire = 0.001  # Valeur minimale
            if volume_unitaire <= 0:
                volume_unitaire = 0.001  # Valeur minimale
                
            # Calcul sécurisé du ratio
            ratio_densite = poids_unitaire / volume_unitaire
            
            # Limiter les valeurs extrêmes
            ratio_densite = max(0.001, min(ratio_densite, 1000))
            
            return ratio_densite
            
        except (ZeroDivisionError, TypeError, AttributeError) as e:
            print(f"⚠️ Erreur calcul efficacité produit: {e}")
...
class SolutionTransport:
    ...
    def _recalculer_cout_total(self):
        """Recalcule le coût total avec protection contre erreurs"""
        try:
            if not hasattr(self, 'voyages') or not self.voyages:
                print("⚠️ Aucun voyage pour recalcul coût")
                return
            ...
            for vehicule_id, voyages_vehicule in voyages_par_vehicule.items():
                try:
                    ...
                    # Protection contre valeurs manquantes
                    cout_fixe_jour = vehicule.get('cout_fixe_jour', 400)
                    cout_variable_km = vehicule.get('cout_variable_km', 0.8)
                    
                    if cout_fixe_jour <= 0:
                        cout_fixe_jour = 400
                    if cout_variable_km <= 0:
                        cout_variable_km = 0.8
                    
                    nb_jours_utilisation = self._calculer_jours_utilisation_secure(voyages_vehicule)
                    distance_totale = self._calculer_distance_totale_secure(voyages_vehicule)
                    
                    # Calculs sécurisés
                    cout_fixe_vehicule = cout_fixe_jour * nb_jours_utilisation
                    cout_variable_vehicule = distance_totale * cout_variable_km
                    
                    cout_vehicule = cout_fixe_vehicule + cout_variable_vehicule
                    cout_total += cout_vehicule
                    
                except Exception as e:
                    print(f"⚠️ Erreur calcul coût véhicule {vehicule_id}: {e}")
                    cout_total += 1000  # Coût par défaut
            
            # Validation finale
            if cout_total <= 0:
                cout_total = self.cout_initial if hasattr(self, 'cout_initial') else 1000
                
            self.cout_total = cout_total
            
        except Exception as e:
            print(f"❌ Erreur majeure recalcul coût: {e}")
            if hasattr(self, 'cout_initial'):
                self.cout_total = self.cout_initial
            else:
                self.cout_total = 1000
    ...
    def _calculer_jours_utilisation(self, voyages_vehicule):
        """Calcule le nombre de jours d'utilisation avec protection"""
        try:
            if not voyages_vehicule or len(voyages_vehicule) == 0:
            ...
            for voyage in voyages_vehicule:
                ...
                date_depart = timing.get('date_depart')
                
                if date_depart and isinstance(date_depart, str):
                    dates_utilisees.add(date_depart)
            
            nb_jours = len(dates_utilisees)
            
            # Validation
            if nb_jours <= 0:
                return 1
            if nb_jours > 365:  # Limite raisonnable
                return 7  # Défaut pour une semaine
                
            return nb_jours
            
        except Exception as e:
            print(f"⚠️ Erreur calcul jours utilisation: {e}")
            return 1


    def _calculer_distance_totale(self, voyages_vehicule):
        """Calcule la distance totale avec protection"""
        try:
            ...
            for i, voyage in enumerate(voyages_vehicule):
                timing = voyage.get('timing', {})
                distance_km = timing.get('distance_km', 800)  # Valeur par défaut
                
                # Validation
                if distance_km <= 0:
                    distance_km = 800
                if distance_km > 10000:  # Limite raisonnable
                    distance_km = 800
                
                # Aller-retour sauf pour le dernier voyage
                if i < len(voyages_vehicule) - 1:
                    distance_voyage = distance_km * 2  # Aller-retour
                else:
                    distance_voyage = distance_km  # Aller seulement
                    
                distance_totale += distance_voyage
            
            return max(0, distance_totale)
            
        except Exception as e:
            print(f"⚠️ Erreur calcul distance totale: {e}")
            return 800 
...
def calculer_navettes_optimales_avec_variabilite(self, produits_a_transporter, vehicules_data, date_debut, heure_debut, duree_max_jours=7, variabilite_mode=None):
    ...
    try:
        ...
        if not produits_a_transporter:
            print("❌ Aucun produit à transporter")
            return self._creer_solution_erreur("Aucun produit", variabilite_mode)
        
        # Validation des véhicules
        vehicules_valides = _valider_vehicules_data(vehicules_data)
        if not vehicules_valides:
            print("❌ Aucun véhicule valide")
            return self._creer_solution_erreur("Aucun véhicule valide", variabilite_mode)
        ...
        for produit in produits_a_transporter:
            try:
                quantite = float(produit.get('quantite', 0))
                poids_unit = float(produit.get('poids_unitaire', 0))
                volume_unit = float(produit.get('volume_unitaire', 0))
                
                poids_total += quantite * poids_unit
                volume_total += quantite * volume_unit
            except (ValueError, TypeError) as e:
                print(f"⚠️ Erreur produit {produit.get('nom', 'INCONNU')}: {e}")
                continue
        
        if poids_total <= 0 and volume_total <= 0:
            print("❌ Aucune charge à transporter")
            return self._creer_solution_erreur("Charge nulle", variabilite_mode)
        ...
        print(f"\n=== OPTIMISATION CORRIGÉE AVEC VARIABILITÉ ===")
        print(f"⏰ CONTRAINTE: {duree_max_jours} jours max")
        print(f"🎲 MODE: {variabilite_mode or 'STANDARD'}")
        print(f"📦 CHARGE: {poids_total:.1f}t, {volume_total:.1f}m³")
        
        # Appliquer stratégie de variabilité sécurisée
        vehicules_tries = self._appliquer_strategie_variabilite_securisee(vehicules_valides, variabilite_mode)
        
        # Chercher solution
        solution_optimale = self._chercher_combinaison_optimale_securisee(
            demande_equivalente, 
            vehicules_tries,
            date_debut,
            heure_debut, 
        ...
        if not solution_optimale:
            print("⚠️ Aucune solution trouvée - Solution par défaut")
            return self._creer_solution_defaut(demande_equivalente, vehicules_tries[0], variabilite_mode)
        
        return solution_optimale
        
    except Exception as e:
        print(f"❌ ERREUR MAJEURE calculer_navettes_optimales: {e}")
        return self._creer_solution_erreur(str(e), variabilite_mode)
...
def main_complet_TOUTES_CORRECTIONS():
    ...
    try:
        ...
        return None
def _valider_vehicules_data(vehicules_data):
    """Valide et nettoie les données des véhicules"""
    vehicules_valides = []
    
    for vehicule in vehicules_data:
        try:
            # Validation des champs obligatoires
            nom = vehicule.get('nom', f'Véhicule_{len(vehicules_valides)+1}')
            
            # Validation des capacités
            capacite_poids = float(vehicule.get('capacite_poids_max', 0))
            capacite_volume = float(vehicule.get('capacite_volume_max', 0))
            
            if capacite_poids <= 0 and capacite_volume <= 0:
                print(f"⚠️ Véhicule {nom} ignoré: capacités nulles")
                continue
            
            # Correction des valeurs manquantes ou aberrantes
            if capacite_poids <= 0:
                capacite_poids = 15.0  # Défaut raisonnable
            if capacite_volume <= 0:
                capacite_volume = 60.0  # Défaut raisonnable
                
            # Validation des coûts
            cout_fixe = float(vehicule.get('cout_fixe_jour', 400))
            cout_variable = float(vehicule.get('cout_variable_km', 0.8))
            vitesse = float(vehicule.get('vitesse_kmh', 80))
            ...
            # Créer véhicule validé
            vehicule_valide = {
                'nom': nom,
                'type': vehicule.get('type', 'STANDARD'),
                'capacite_poids_max': capacite_poids,
                'capacite_volume_max': capacite_volume,
                'vitesse_kmh': vitesse,
                'cout_fixe_jour': cout_fixe,
                'cout_variable_km': cout_variable,
                'quantite': int(vehicule.get('quantite', 1))
            }
            
            vehicules_valides.append(vehicule_valide)
            
        except (ValueError, TypeError) as e:
            print(f"⚠️ Erreur validation véhicule {vehicule.get('nom', 'INCONNU')}: {e}")
            continue
    
    if not vehicules_valides:
        print("❌ Aucun véhicule valide - Création d'un véhicule par défaut")
        vehicules_valides.append({
            'nom': 'Véhicule Standard Défaut',
            'type': 'STANDARD',
            'capacite_poids_max': 20.0,
            'capacite_volume_max': 80.0,
            'vitesse_kmh': 80,
    ...
    print(f"✅ {len(vehicules_valides)} véhicule(s) validé(s)")
    return vehicules_valides

def _selectionner_meilleur_vehicule_securise(self, vehicules_data, demande, date_debut, heure_debut, duree_max_jours):
    """Sélection sécurisée du meilleur véhicule"""
    try:
        # Valider les véhicules d'abord
        vehicules_valides = _valider_vehicules_data(vehicules_data)
        
        if not vehicules_valides:
            print("❌ Aucun véhicule valide disponible")
            return None
        ...
        for vehicule in vehicules_valides:
            try:
                result = self._vehicule_peut_reussir_seul_avec_calcul_securise(
                    vehicule, 
                    demande.get('quantite_poids', 0), 
                    demande.get('quantite_volume', 0),
                    demande.get('distance_km', 800), 
                    duree_max_jours
                )
                ...
                print(f"⚠️ Erreur évaluation véhicule {vehicule['nom']}: {e}")
                continue
        ...
        if meilleur_vehicule:
            ...
            print("⚠️ Aucun véhicule ne peut réussir seul - Retour solution par défaut")
            return {
                'vehicule': vehicules_valides[0],  # Prendre le premier valide
                'cout_total': 2000,  # Coût pénalisé
                'duree_reelle_jours': duree_max_jours,
                'type_solution': 'vehicule_unique'
            }
        
    except Exception as e:
        print(f"❌ Erreur sélection véhicule: {e}")
        return None
