...
class OptimisateurNavettes:
    ...
    def calculer_navettes_optimales(self, produits_a_transporter, vehicules_data, date_debut, heure_debut, duree_max_jours=7):
        ...
        for vehicule in vehicules_data:
            ...
            if analyse['respect_contrainte']:
                ...
                print(f"  Coût fixe total: {analyse['cout_fixe_total']:.2f}DA")
                print(f"  Coût variable total: {analyse['cout_variable_total']:.2f}DA")
                print(f"  💰 COÛT TOTAL: {analyse['cout_total']:.2f}DA")
                print(f"  📊 Coût par véhicule: {analyse['cout_par_vehicule']:.2f}DA")
            else:
        ...
        print(f"💰 COÛT TOTAL: {vehicule_optimal['cout_total']:.2f}DA")
        ...
        for analyse in vehicules_viables:
            if analyse != vehicule_optimal:
                ...
                print(f"  vs {analyse['vehicule']['nom']}: +{economie:.2f}DA ({economie/vehicule_optimal['cout_total']*100:.1f}% plus cher)")
    ...
    def _calculer_efficacite_produit(self, produit):
        """Calcule l'efficacité d'un produit pour l'algorithme sac à dos"""
        # Densité : poids par volume (produits compacts = efficaces)
        densite = produit['poids_unitaire'] / max(produit['volume_unitaire'], 0.001)
        
        # Critères d'efficacité :
        # 1. Densité élevée = optimise l'espace volume
        # 2. Poids élevé = optimise l'espace poids
        # 3. Équilibre entre les deux contraintes
        
        efficacite_volume = densite  # Plus dense = mieux pour le volume
        efficacite_poids = 1.0 / max(produit['poids_unitaire'], 0.001)  # Plus léger = plus d'unités possibles
        
        # Favoriser les produits qui optimisent la contrainte la plus restrictive
        # Produits denses pour maximiser l'utilisation
        efficacite = densite + (produit['poids_unitaire'] * 0.1)  # Léger bonus pour les produits lourds
        
        return efficacite

    def _selectionner_produits_melange(self, pool_produits, capacite_poids_max, capacite_volume_max, voyage_num):
        """
        Sélectionne les produits selon la stratégie de mélange optimal avec sac à dos amélioré:
        1. Remplissage homogène prioritaire (un seul type de produit si possible)
        2. Algorithme sac à dos glouton bi-dimensionnel optimisé
        """
        ...
        # PHASE 2: Algorithme sac à dos glouton bi-dimensionnel optimisé
        print(f"    🎒 PHASE 2: Sac à dos glouton bi-dimensionnel (remplissage homogène <70%)")
        
        # Calculer l'efficacité de chaque produit et afficher
        efficacites = {}
        for nom_produit, info in produits_disponibles.items():
            efficacite = self._calculer_efficacite_produit(info['produit'])
            efficacites[nom_produit] = efficacite
            print(f"      📊 {nom_produit}: efficacité {efficacite:.2f}")
        
        # Trier les produits par efficacité décroissante (sac à dos glouton)
        produits_tries = sorted(produits_disponibles.items(), 
                            key=lambda x: efficacites[x[0]],
                            reverse=True)
        
        print(f"      🔄 Ordre de sélection (efficacité décroissante):")
        for nom_produit, info in produits_tries:
            print(f"        {nom_produit} (efficacité: {efficacites[nom_produit]:.2f})")
        
        # Algorithme sac à dos glouton : prendre le maximum de chaque produit dans l'ordre d'efficacité
        for nom_produit, info in produits_tries:
            produit = info['produit']
            quantite_disponible = info['quantite_restante']
            ...
            if max_unites > 0:
                ...
                print(f"      ✗ {nom_produit}: aucune unité ne rentre dans l'espace restant")
        
        # Afficher le résumé de l'utilisation finale
        poids_total = capacite_poids_max - capacite_poids_restante
        volume_total = capacite_volume_max - capacite_volume_restante
        utilisation_poids = (poids_total / capacite_poids_max) * 100
        utilisation_volume = (volume_total / capacite_volume_max) * 100
        
        print(f"      📊 Utilisation finale: Poids {utilisation_poids:.1f}%, Volume {utilisation_volume:.1f}%")
    ...
    def _optimiser_chargement_sac_a_dos(self, produits_disponibles, capacite_poids_max, capacite_volume_max):
        """Vrai algorithme sac à dos bi-dimensionnel (poids + volume)"""
        
        # Créer la liste des items disponibles
        items = []
        for nom_produit, info in produits_disponibles.items():
            produit = info['produit']
            quantite_disponible = info['quantite_restante']
            
            for unite in range(min(quantite_disponible, 50)):  # Limiter pour performance
                items.append({
                    'nom': nom_produit,
                    'produit': produit,
                    'poids': produit['poids_unitaire'],
                    'volume': produit['volume_unitaire'],
                    'valeur': self._calculer_valeur_unite(produit),  # Définir une fonction de valeur
                    'index': unite
                })
        
        # Tri par efficacité (valeur/poids et valeur/volume)
        items.sort(key=lambda x: x['valeur'] / (x['poids'] + x['volume']), reverse=True)
        ...
        for item in items:
            if (poids_utilise + item['poids'] <= capacite_poids_max and 
                volume_utilise + item['volume'] <= capacite_volume_max):
                
                selection.append(item)
                poids_utilise += item['poids']
                volume_utilise += item['volume']
        
        # Consolider par produit
        return self._consolider_selection(selection)

    def _calculer_valeur_unite(self, produit):
        """Calcule la valeur d'une unité de produit"""
        # Critères : priorité par densité et facilité de transport
        densite = produit['poids_unitaire'] / max(produit['volume_unitaire'], 0.001)
        
        # Plus dense = plus de valeur (optimise l'espace)
        valeur = densite * 100
    ...
    def _analyser_vehicule_demande_partielle(self, demande_restante, vehicule, duree_max_jours):
        ...
        efficacite_poids = min(poids_transportable / (nb_voyages_realisables * vehicule['capacite_poids_max']), 1.0)
        efficacite_volume = min(volume_transportable / (nb_voyages_realisables * vehicule['capacite_volume_max']), 1.0)
        efficacite_moyenne = (efficacite_poids + efficacite_volume) / 2
        
        # Proportion de la demande totale que ce véhicule peut satisfaire
        proportion_demande = poids_transportable / demande_restante['quantite_poids']
        ...
        return {
            ...
            'efficacite_transport': proportion_demande,
            'efficacite_utilisation': efficacite_moyenne,
            'proportion_demande_satisfaite': proportion_demande
        }
    def _detecter_derniers_voyages(self, demandes_restantes, seuil_pourcentage=20):
        """
        Détecte si on est dans la phase des derniers voyages (demande < seuil% de la demande initiale)
        """
        poids_restant = sum(p['quantite'] * p['poids_unitaire'] for p in demandes_restantes if p['quantite'] > 0)
        
        # Calculer le poids initial (approximation basée sur les quantités initiales max observées)
        # Cette approximation est volontairement simple pour éviter de stocker l'état initial
        poids_approximatif_initial = poids_restant * 3  # Estimation grossière
        
        pourcentage_restant = (poids_restant / max(poids_approximatif_initial, poids_restant)) * 100
        
        return pourcentage_restant <= seuil_pourcentage, poids_restant
    def _valider_gain_reel(self, vehicule_selectionne, analyses_vehicules):
        """
        Valide que le véhicule sélectionné apporte un gain réel par rapport aux alternatives
        """
        if len(analyses_vehicules) <= 1:
            return True  # Pas d'alternative à comparer
        
        # Trouver la meilleure alternative
        autres_vehicules = [a for a in analyses_vehicules 
                        if a is not None and a != vehicule_selectionne 
                        and a['poids_transportable'] > 0]
        
        if not autres_vehicules:
            return True
        
        meilleure_alternative = min(autres_vehicules, key=lambda x: x['cout_par_tonne'])
        
        # Le gain doit être significatif (>2%) pour justifier le changement
        gain_pourcentage = (meilleure_alternative['cout_par_tonne'] - vehicule_selectionne['cout_par_tonne']) / meilleure_alternative['cout_par_tonne']
        
        if gain_pourcentage > 0.02:  # Gain > 2%
            print(f"    ✅ Gain réel validé: {gain_pourcentage*100:.1f}% vs {meilleure_alternative['vehicule']['nom']}")
            return True
        else:
            print(f"    ⚠️ Gain marginal: {gain_pourcentage*100:.1f}% - Utilisation alternative recommandée")
            return False

    def _selectionner_vehicule_cout_minimal(self, analyses_vehicules, demandes_restantes, iteration):
        """
        Sélectionne le véhicule optimal avec réévaluation complète et seuils d'efficacité
        """
        ...
        # Calculer la taille de la demande restante
        poids_total_restant = sum(p['quantite'] * p['poids_unitaire'] for p in demandes_restantes if p['quantite'] > 0)
        
        # CRÉER LA DEMANDE ÉQUIVALENTE LOCALEMENT
        demande_equivalente = {
            'quantite_poids': poids_total_restant,
            'quantite_volume': sum(p['quantite'] * p['volume_unitaire'] for p in demandes_restantes if p['quantite'] > 0),
            'distance_km': demandes_restantes[0]['distance_km'],
            'vitesse_kmh': demandes_restantes[0]['vitesse_kmh']
        }
        
        # SEUILS D'EFFICACITÉ selon la taille de la demande
        if poids_total_restant > 80:  # Grosse demande
            seuil_efficacite_min = 0.60  # 60% minimum
            priorite = "cout_par_tonne"  # Prioriser le coût
        elif poids_total_restant > 30:  # Demande moyenne  
            seuil_efficacite_min = 0.50  # 50% minimum
            priorite = "equilibre"  # Équilibrer coût et efficacité
        else:  # Petite demande (derniers voyages)
            seuil_efficacite_min = 0.30  # 30% minimum (plus flexible)
            priorite = "efficacite"  # Prioriser l'efficacité
        
        print(f"    📊 RÉÉVALUATION ITÉRATION {iteration}:")
        print(f"    Demande restante: {poids_total_restant:.2f}t")
        print(f"    Seuil d'efficacité: {seuil_efficacite_min*100:.0f}%")
        print(f"    Priorité: {priorite}")
        
        # OPTIMISATION SPÉCIALE POUR PETITES CHARGES
        if poids_total_restant <= 15:  # Très petite charge
            print(f"    🎯 OPTIMISATION PETITE CHARGE: {poids_total_restant:.1f}t")
            
            # Recalculer avec coût réel pour tous les véhicules
            for analyse in analyses_valides:
                cout_reel = self._calculer_cout_reel_petite_charge(analyse['vehicule'], demande_equivalente)
                if cout_reel:
                    analyse['cout_par_tonne_reel'] = cout_reel['cout_par_tonne_reel']
                    print(f"      {analyse['vehicule']['nom']}: Coût réel {analyse['cout_par_tonne_reel']:.0f} DA/t")
                else:
                    analyse['cout_par_tonne_reel'] = analyse['cout_par_tonne']
            
            # Sélectionner selon le coût réel
            vehicule_optimal = min(analyses_valides, 
                                key=lambda x: x.get('cout_par_tonne_reel', x['cout_par_tonne']))
            
            print(f"    🎯 VÉHICULE OPTIMAL PETITE CHARGE: {vehicule_optimal['vehicule']['nom']}")
            print(f"      Coût réel par tonne: {vehicule_optimal.get('cout_par_tonne_reel', vehicule_optimal['cout_par_tonne']):.0f} DA/t")
            
            return vehicule_optimal
        
        # SUITE DU CODE EXISTANT POUR LES GROSSES CHARGES...
        # Filtrer les véhicules qui respectent le seuil d'efficacité
        candidats_efficaces = []
        candidats_sous_seuil = []
        
        for analyse in analyses_valides:
            efficacite = analyse.get('efficacite_utilisation', 0.5)  # Valeur par défaut si pas calculée
            
            print(f"      {analyse['vehicule']['nom']}:")
            print(f"        Coût/tonne: {analyse['cout_par_tonne']:.2f}DA/t")
            print(f"        Efficacité: {efficacite*100:.1f}%")
            print(f"        Peut transporter: {analyse['poids_transportable']:.1f}t")
            
            if efficacite >= seuil_efficacite_min:
                candidats_efficaces.append(analyse)
                print(f"        ✅ Respecte le seuil d'efficacité")
            else:
                candidats_sous_seuil.append(analyse)
                print(f"        ⚠️ Sous le seuil d'efficacité")
        
        # SÉLECTION selon la priorité et les candidats disponibles
        candidats_finaux = candidats_efficaces if candidats_efficaces else candidats_sous_seuil
        
        if priorite == "cout_par_tonne":
            vehicule_optimal = min(candidats_finaux, key=lambda x: x['cout_par_tonne'])
        elif priorite == "equilibre":
            def score_equilibre(analyse):
                cout_normalise = analyse['cout_par_tonne'] / 200
                efficacite_normalise = (1 - analyse.get('efficacite_utilisation', 0.5))
                return cout_normalise + efficacite_normalise
            vehicule_optimal = min(candidats_finaux, key=score_equilibre)
        else:  # priorite == "efficacite"
            vehicule_optimal = max(candidats_finaux, 
                                key=lambda x: (x.get('efficacite_utilisation', 0.5), -x['cout_par_tonne']))
        
        print(f"    🎯 VÉHICULE SÉLECTIONNÉ: {vehicule_optimal['vehicule']['nom']}")
        print(f"      Coût par tonne: {vehicule_optimal['cout_par_tonne']:.2f}DA/t")
        efficacite_affichage = vehicule_optimal.get('efficacite_utilisation', 0.5)
        print(f"      Efficacité: {efficacite_affichage*100:.1f}%")
        print(f"      Raison: {priorite}")
        
        # Valider que le choix apporte un gain réel
        gain_valide = self._valider_gain_reel(vehicule_optimal, analyses_valides)
        if not gain_valide and len(analyses_valides) > 1:
            alternative = min([a for a in analyses_valides if a != vehicule_optimal], 
                            key=lambda x: x['cout_par_tonne'])
            print(f"    🔄 CHANGEMENT: Utilisation de {alternative['vehicule']['nom']} (gain non significatif)")
            vehicule_optimal = alternative
        
        return vehicule_optimal
    def _calculer_cout_reel_petite_charge(self, vehicule, demande_equivalente):
        """Calcul précis pour dernières charges < 50% capacité"""
        
        poids_restant = demande_equivalente['quantite_poids']
        volume_restant = demande_equivalente['quantite_volume']
        
        # VÉRIFIER SI LE VÉHICULE PEUT TRANSPORTER LA CHARGE EN 1 VOYAGE
        peut_transporter_poids = poids_restant <= vehicule['capacite_poids_max']
        peut_transporter_volume = volume_restant <= vehicule['capacite_volume_max']
        
        if peut_transporter_poids and peut_transporter_volume:
            # Un seul voyage suffit
            nb_voyages_reel = 1
            
            # Calculer le coût exact pour 1 voyage
            distance_totale = demande_equivalente['distance_km'] * 2  # A-R
            vitesse = vehicule.get('vitesse_kmh', demande_equivalente['vitesse_kmh'])
            duree_voyage = (distance_totale / vitesse) + 3  # +3h ops
            nb_jours = max(1, math.ceil(duree_voyage / 8))  # Minimum 1 jour
            
            cout_fixe = nb_jours * vehicule['cout_fixe_jour']
            cout_variable = distance_totale * vehicule['cout_variable_km']
            cout_total = cout_fixe + cout_variable
            
            print(f"        → Calcul réel: {nb_voyages_reel} voyage, {nb_jours} jour(s), {cout_total:.0f} DA")
            
            return {
                'nb_voyages_reel': nb_voyages_reel,
                'cout_total_reel': cout_total,
                'cout_par_tonne_reel': cout_total / poids_restant
            }
        else:
            # Plus d'un voyage nécessaire - calcul classique
            nb_voyages_poids = math.ceil(poids_restant / vehicule['capacite_poids_max'])
            nb_voyages_volume = math.ceil(volume_restant / vehicule['capacite_volume_max'])
            nb_voyages_reel = max(nb_voyages_poids, nb_voyages_volume)
            
            # Calcul pour voyages multiples
            distance_totale = demande_equivalente['distance_km'] * (2 * nb_voyages_reel - 1)  # Dernier sans retour
            vitesse = vehicule.get('vitesse_kmh', demande_equivalente['vitesse_kmh'])
            duree_totale = (distance_totale / vitesse) + (nb_voyages_reel * 3)  # +3h ops par voyage
            nb_jours = max(1, math.ceil(duree_totale / 8))
            
            cout_fixe = nb_jours * vehicule['cout_fixe_jour']
            cout_variable = distance_totale * vehicule['cout_variable_km']
            cout_total = cout_fixe + cout_variable
            
            print(f"        → Calcul réel: {nb_voyages_reel} voyages, {nb_jours} jour(s), {cout_total:.0f} DA")
    ...
    def calculer_navettes_optimales_flotte_heterogene(self, produits_a_transporter, vehicules_data, date_debut, heure_debut, duree_max_jours=7):
        """
        Optimisation avec flotte hétérogène - sélection itérative par coût minimal AVEC LIMITATION PAR TYPE
        """
        ...
        # NOUVEAU: Tracker la disponibilité des véhicules
        vehicules_disponibles = []
        for vehicule in vehicules_data:
            for i in range(vehicule['quantite_disponible']):
                vehicule_instance = copy.deepcopy(vehicule)
                vehicule_instance['instance_id'] = f"{vehicule['nom']}_INSTANCE_{i+1}"
                vehicule_instance['nom_original'] = vehicule['nom']
                vehicules_disponibles.append(vehicule_instance)
        
        print(f"\n=== OPTIMISATION FLOTTE HÉTÉROGÈNE AVEC LIMITATIONS ===")
        print(f"Contrainte temporelle globale: {duree_max_jours} jours")
        print(f"🚛 FLOTTE DISPONIBLE TOTALE: {len(vehicules_disponibles)} véhicules")
        
        # Afficher la composition de la flotte
        vehicules_par_type = {}
        for vehicule in vehicules_disponibles:
            nom_original = vehicule['nom_original']
            if nom_original not in vehicules_par_type:
                vehicules_par_type[nom_original] = 0
            vehicules_par_type[nom_original] += 1
        
        for nom_type, count in vehicules_par_type.items():
            print(f"  - {nom_type}: {count} véhicule(s) disponible(s)")
        
        while self._reste_des_demandes(demandes_restantes):
            print(f"\n--- ITÉRATION {iteration} ---")
            
            if not vehicules_disponibles:
                print("❌ AUCUN VÉHICULE DISPONIBLE pour les demandes restantes")
                break
            
            # Calculer demande équivalente restante
            demande_restante = self._calculer_demande_equivalente(demandes_restantes)
            
            print(f"📊 VÉHICULES ENCORE DISPONIBLES: {len(vehicules_disponibles)}")
            for nom_type, count in vehicules_par_type.items():
                disponibles = sum(1 for v in vehicules_disponibles if v['nom_original'] == nom_type)
                utilises = count - disponibles
                print(f"  - {nom_type}: {disponibles}/{count} disponibles ({utilises} utilisés)")
            
            # Analyser tous les véhicules DISPONIBLES pour cette demande restante
            analyses_vehicules = []
            for vehicule in vehicules_disponibles:
            ...
            vehicule_selectionne = self._selectionner_vehicule_cout_minimal(analyses_vehicules, demandes_restantes, iteration)
            ...
            # NOUVEAU: Retirer le véhicule utilisé de la liste des disponibles
            vehicule_utilise = vehicule_selectionne['vehicule']
            vehicules_disponibles = [v for v in vehicules_disponibles if v['instance_id'] != vehicule_utilise['instance_id']]
            
            print(f"🚛 VÉHICULE UTILISÉ: {vehicule_utilise['instance_id']}")
            print(f"📉 VÉHICULES RESTANTS: {len(vehicules_disponibles)}")
        ...
        # Vérifier si toutes les demandes ont été satisfaites
        demandes_satisfaites = not self._reste_des_demandes(demandes_restantes)
        
        if not demandes_satisfaites:
            print(f"\n⚠️ ATTENTION: Demandes non satisfaites avec la flotte limitée!")
            print(f"Véhicules restants: {len(vehicules_disponibles)}")
            print(f"Demandes restantes:")
            for produit in demandes_restantes:
                if produit['quantite'] > 0:
                    print(f"  - {produit['nom']}: {produit['quantite']} unités restantes")
        
        return {
            'flotte_utilisee': flotte_utilisee,
            'cout_total_global': cout_total_global,
            'demandes_satisfaites': demandes_satisfaites,
            'nb_vehicules_utilises': len(flotte_utilisee),
            'vehicules_restants': len(vehicules_disponibles),
            'composition_flotte_utilisee': self._analyser_composition_flotte(flotte_utilisee)
        }
    def _analyser_composition_flotte(self, flotte_utilisee):
        """Analyse la composition de la flotte utilisée"""
        composition = {}
        for vehicule_info in flotte_utilisee:
            nom_original = vehicule_info['vehicule_utilise'].get('nom_original', vehicule_info['vehicule_utilise']['nom'])
            if nom_original not in composition:
                composition[nom_original] = {
                    'count': 0,
                    'cout_total': 0,
                    'voyages_total': 0
                }
            composition[nom_original]['count'] += 1
            composition[nom_original]['cout_total'] += vehicule_info['cout_vehicule']
            composition[nom_original]['voyages_total'] += vehicule_info['nb_voyages_realises']
...
def main_test_basique():
    """
    Test avec flotte de véhicules algériens - données réalistes
    """
    print("="*100)
    print("🚀 TEST FLOTTE VÉHICULES ALGÉRIENS - OPTIMISATION TRANSPORT HÉTÉROGÈNE")
    print("="*100)
    
    # ✅ DONNÉES DE TEST - PRODUITS IDENTIQUES
    
    # 1. Produits à transporter (5 types identiques)
    produits_test = [
        {
    ...
    # 2. FLOTTE ALGÉRIENNE - Spécifications réalistes
    vehicules_algeriens = [
        {
            'nom': 'PLATEAU_4X2',
            'type': 'LEGER',
            'capacite_poids_max': 8.0,
            'capacite_volume_max': 35.0,
            'vitesse_kmh': 85,
            'cout_fixe_jour': 150,
            'cout_variable_km': 280,
        ...
        },
        {
            ...
            'capacite_volume_max': 45.0,
            'vitesse_kmh': 75,
            'cout_fixe_jour': 200,
            'cout_variable_km': 400,
            'quantite_disponible': 1
        },
        {
            'nom': 'SEMI_REMORQUE_6X4',
            'type': 'LOURD_ROUTIER',
            'capacite_poids_max': 32.0,
            'capacite_volume_max': 85.0,
            'vitesse_kmh': 80,
            'cout_fixe_jour': 280,
            'cout_variable_km': 180,
            'quantite_disponible': 2
        },
        {
            'nom': 'SEMI_REMORQUE_6X6',
            'type': 'LOURD_TOUT_TERRAIN',
            'capacite_poids_max': 28.0,
        ...
        {
            'nom': 'PORTE_ENGIN_6X4',
            'type': 'SPECIALISE_ROUTIER',
            'capacite_poids_max': 25.0,
        ...
        {
            'nom': 'PORTE_ENGIN_6X6',
            'type': 'SPECIALISE_TOUT_TERRAIN',
            'capacite_poids_max': 22.0,
            'capacite_volume_max': 55.0,
            'vitesse_kmh': 65,
            'cout_fixe_jour': 350,
            'cout_variable_km': 400,
            'quantite_disponible': 2
    ...
    print(f"🚛 Flotte algérienne: {len(vehicules_algeriens)} types de véhicules")
    ...
    print(f"\n🚛 FLOTTE ALGÉRIENNE DISPONIBLE:")
    for vehicule in vehicules_algeriens:
        ...
        print(f"    Coûts: {vehicule['cout_fixe_jour']} DA/jour + {vehicule['cout_variable_km']} DA/km")
        print(f"    Quantité disponible: {vehicule['quantite_disponible']} véhicule(s)")
        
        # Calcul coût estimé pour 800km aller-retour
        cout_variable_voyage = vehicule['cout_variable_km'] * 1600  # 800km x 2
        cout_fixe_2jours = vehicule['cout_fixe_jour'] * 2  # Estimation 2 jours
        cout_total_estime = cout_variable_voyage + cout_fixe_2jours
        print(f"    💰 Coût estimé voyage A-R: {cout_total_estime:.0f} DA")

    # ✅ LANCEMENT DE L'OPTIMISATION
    print(f"\n" + "="*100)
    print("🔍 LANCEMENT DE L'OPTIMISATION FLOTTE HÉTÉROGÈNE ALGÉRIENNE")
    print("="*100)
    ...
    try:
        ...
        # Lancer l'optimisation avec flotte hétérogène algérienne
        ...
        # Le reste du code de traitement des résultats continue ici...
        # (tout le code existant pour afficher les résultats)
        if resultats_heterogene and resultats_heterogene['demandes_satisfaites']:
            print(f"\n" + "="*100)
            print("🎯 RÉSULTATS OPTIMISATION FLOTTE ALGÉRIENNE")
            print("="*100)
            
            # Résultats principaux
            cout_total_da = resultats_heterogene['cout_total_global']
            cout_total_euro = cout_total_da / 150  # Conversion approximative DA vers EUR
            nb_vehicules_utilises = resultats_heterogene['nb_vehicules_utilises']
            flotte_utilisee = resultats_heterogene['flotte_utilisee']
            
            print(f"\n✅ OPTIMISATION RÉUSSIE!")
            print(f"💰 Coût total: {cout_total_da:.0f} DA ({cout_total_euro:.0f} EUR)")
            ...
            print(f"📈 Coût par tonne: {cout_total_da/poids_total:.0f} DA/t ({cout_total_euro/poids_total:.0f} EUR/t)")
            
            print(f"\n📊 COMPOSITION DE LA FLOTTE UTILISÉE:")
            composition = resultats_heterogene['composition_flotte_utilisee']
            for nom_type, stats in composition.items():
                print(f"  {nom_type}:")
                print(f"    - Véhicules utilisés: {stats['count']}")
                print(f"    - Coût total du type: {stats['cout_total']:.0f} DA")
                print(f"    - Voyages total du type: {stats['voyages_total']}")
                print(f"    - Coût moyen par véhicule: {stats['cout_total']/stats['count']:.0f} DA")
            
            print(f"\n🚛 VÉHICULES NON UTILISÉS: {resultats_heterogene['vehicules_restants']}")
            
            # Détails par véhicule utilisé
            print(f"\n📋 DÉTAILS DE LA FLOTTE ALGÉRIENNE UTILISÉE:")
            for i, vehicule_info in enumerate(flotte_utilisee, 1):
                vehicule_nom = vehicule_info['vehicule_utilise']['nom']
                vehicule_type = vehicule_info['vehicule_utilise']['type']
                nb_voyages = vehicule_info['nb_voyages_realises']
                cout_vehicule_da = vehicule_info['cout_vehicule']
                cout_vehicule_eur = cout_vehicule_da / 150
                produits_transportes = vehicule_info['produits_transportes']
                
                print(f"\n  🚚 Véhicule {i}: {vehicule_nom} ({vehicule_type})")
                print(f"    💰 Coût: {cout_vehicule_da:.0f} DA ({cout_vehicule_eur:.0f} EUR)")
                ...
                for nom_produit, quantite in produits_transportes.items():
                    ...
                    poids_total_vehicule += poids_transporte
                    volume_total_vehicule += volume_transporte
                    
                    print(f"      - {nom_produit}: {quantite} unités ({poids_transporte:.2f}t, {volume_transporte:.2f}m³)")
                
                # Efficacité du véhicule
                capacite_poids = vehicule_info['vehicule_utilise']['capacite_poids_max'] * nb_voyages
                capacite_volume = vehicule_info['vehicule_utilise']['capacite_volume_max'] * nb_voyages
                efficacite_poids = (poids_total_vehicule / capacite_poids) * 100 if capacite_poids > 0 else 0
                efficacite_volume = (volume_total_vehicule / capacite_volume) * 100 if capacite_volume > 0 else 0
                
                print(f"    📊 Efficacité: Poids {efficacite_poids:.1f}%, Volume {efficacite_volume:.1f}%")
                print(f"    💰 Coût par tonne: {cout_vehicule_da/max(poids_total_vehicule, 0.1):.0f} DA/t")
            
            # Statistiques globales et comparaisons
            total_voyages = sum(v['nb_voyages_realises'] for v in flotte_utilisee)
            cout_moyen_par_vehicule_da = cout_total_da / nb_vehicules_utilises
            
            print(f"\n📊 STATISTIQUES GLOBALES:")
            print(f"  🔄 Total voyages: {total_voyages}")
            print(f"  💰 Coût moyen par véhicule: {cout_moyen_par_vehicule_da:.0f} DA")
            print(f"  🚛 Types de véhicules utilisés:")
            
            types_utilises = {}
            for vehicule_info in flotte_utilisee:
                nom = vehicule_info['vehicule_utilise']['nom']
                types_utilises[nom] = types_utilises.get(nom, 0) + 1
            
            for nom_vehicule, count in types_utilises.items():
                print(f"    - {nom_vehicule}: {count} véhicule(s)")
            
            # Analyse de la pertinence de la sélection
            print(f"\n🔍 ANALYSE DE LA SÉLECTION:")
            vehicules_non_utilises = [v for v in vehicules_algeriens 
                                    if v['nom'] not in [info['vehicule_utilise']['nom'] for info in flotte_utilisee]]
            
            if vehicules_non_utilises:
                print(f"  📋 Véhicules non sélectionnés:")
                for vehicule in vehicules_non_utilises:
                    cout_variable_voyage = vehicule['cout_variable_km'] * 1600
                    cout_fixe_2jours = vehicule['cout_fixe_jour'] * 2
                    cout_estime = cout_variable_voyage + cout_fixe_2jours
                    print(f"    - {vehicule['nom']}: Coût estimé {cout_estime:.0f} DA/voyage")
                    print(f"      Raison probable: Coût/km élevé ({vehicule['cout_variable_km']} DA/km)")
        ...
        elif resultats_heterogene and not resultats_heterogene['demandes_satisfaites']:
            print(f"\n" + "="*100)
            print("⚠️ OPTIMISATION PARTIELLE - FLOTTE LIMITÉE INSUFFISANTE")
            print("="*100)
            
            cout_total_da = resultats_heterogene['cout_total_global']
            cout_total_euro = cout_total_da / 150
            nb_vehicules_utilises = resultats_heterogene['nb_vehicules_utilises']
            
            print(f"\n⚠️ OPTIMISATION PARTIELLE!")
            print(f"💰 Coût total: {cout_total_da:.0f} DA ({cout_total_euro:.0f} EUR)")
            print(f"🚛 Nombre de véhicules utilisés: {nb_vehicules_utilises}")
            print(f"🚛 Véhicules restants: {resultats_heterogene['vehicules_restants']}")
            print(f"❌ Toutes les demandes satisfaites: NON")
            
            print(f"\n📊 COMPOSITION DE LA FLOTTE UTILISÉE:")
            composition = resultats_heterogene['composition_flotte_utilisee']
            for nom_type, stats in composition.items():
                print(f"  {nom_type}: {stats['count']} véhicule(s) utilisé(s)")
            
            print(f"\n💡 RECOMMANDATIONS:")
            print(f"   - Augmenter le nombre de véhicules SEMI_REMORQUE_6X4 disponibles")
            print(f"   - Ou ajouter plus de véhicules SEMI_REMORQUE_6X6")
            print(f"   - Ou étaler la demande sur une période plus longue")
            ...
            if resultats_heterogene:
                ...
                print(f"Coût total: {resultats_heterogene['cout_total_global']:.0f} DA")
            else:
    ...
    print(f"\n" + "="*100)
    print("🏁 FIN DU TEST FLOTTE ALGÉRIENNE")
    print("="*100)


# ✅ FONCTION PRINCIPALE
if __name__ == "__main__":
    print("🚀 Lancement du test avec flotte véhicules algériens...")
    main_test_basique()
